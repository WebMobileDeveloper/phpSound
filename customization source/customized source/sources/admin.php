...
function getClaims($db,$type){
    $query = "SELECT claim.*, users.email as uEmail, users.username from claim JOIN users ON claim.uid=users.idu WHERE claim.status='{$type}' ORDER BY claim.date ";
    $select_rows = $db->query($query);
    $table = "<table style='text-align: center;'><thead>
                                            <th>No</th>
                                            <th>User Name/<br>Email</th>
                                            <th>Billing Type</th>
                                            <th>Paypal email/<br>bankName</th>
                                            <th>Account<br>Number / Name<br></th>
                                            <th>Additional Info</th>
                                            <th>Claim/Pay Amount</th>
                                            <th>Date</th>
                                            <th>Accept/<br>Decline</th>
                                        </thead>";
    $i = 0;
    //  billingAmount, , date
    while ($row = $select_rows->fetch_assoc()) {
        ...
        $paid = round($row['billingAmount']*0.9 )/ 1000000;
        $id = $row['id'];
        $status = $row['status'];
        $uid = $row['uid'];
        ...
        $tr = "<tr>
                                    <td rowspan='2'>{$i}</td>
                                    <td>{$username}</td>
                                    <td rowspan='2'>{$row['billingtype']}</td>
                                    <td>{$row['email']}</td>
                                    <td>{$row['bankAccountNumber']}</td>
                                    <td rowspan='2'>{$row['AdditionalInfo']}</td>
                                    <td>{$bill}</td>
                                    <td rowspan='2'>{$row['date']}</td>
                                    <td><button style='width:20px !important; padding: 5px 2px 5px 26px;' uid='{$uid}' rowId='{$id}' status='{$status}' bill='{$bill}' class='like-button'></button></td>
                              </tr>";
        $table .= $tr;
        $tr = "<tr>
                                    <td>{$email}</td>
                                    ...
                                    <td>{$paid}</td>
                                    <td><button style='width:30px;' uid='{$uid}' rowId='{$id}' status='{$status}' bill='{$bill}' class='delete-button'></button></td>
                               </tr>";
        $table .= $tr;
    }
    $table .= "</table>";
...
function getEarnings($db){
    ...
    while ($row = $select_rows->fetch_assoc()) {
        $i++;
        $earnings = $row['earnings'] / 1000000;
        $paid = $row['paid']/ 1000000;
        $pending = $row['pending']/ 1000000;
        $available = $earnings-$pending-$paid;
        $id = $row['idu'];
        $email = $row['email'];
        $username = $row['username'];
        $active = $row['active'];
        $tr = "<tr>
                                    ...
                                    <td>{$paid}</td>
                                    <td>{$active}</td>
                                    <td><button style='width:40px !important; padding: 5px 2px 5px 5px;' username='{$username}' useremail='{$email}' uid='{$id}' earnings='{$earnings}' 
                                        available='{$available}' pending='{$pending}' paid='{$paid}' active='{$active}' class='button-edit'>Edit</button></td>
                              </tr>";
        $table .= $tr;
    }
    $table .= "</table>";
...
function PageMain()
{
        ...
        if ($loggedIn['username']) {
            ...
            switch ($_GET['b']) {
                ...
                case 'payments':
                    ...
                    break;
                case 'artistPayments':
                    $TMPL['claims']=getClaims($db,'Pending');
                    $TMPL['Success']=getClaims($db,'Success');
                    $TMPL['Decline']=getClaims($db,'Decline');
                    $TMPL['Earnings']=getEarnings($db);


                    $skin = new skin('admin/manage_artistPayments');
                    $page = '';
                    break;
                case 'pro':
                ...
                case 'site_settings':
                    ...
                    $TMPL['unit_cost']=$settings['unit_cost'];
                    $TMPL['ip_counts']=$settings['ip_counts'];
                    $TMPL['minimum_limit']=$settings['minimum_limit'];
    ...
    $menu = array('' => array('admin_menu_dashboard', ''),
        ...
        '&b=artistPayments' => array('admin_menu_manage_artist_payments', ''),
